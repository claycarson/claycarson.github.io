...
def paginate_indefinitely(table)
  ...
  offset = nil
  loop do
    page = table.records(view: 'Source', offset: offset)
...
travels_records = collect_records('Travels')
travels_records.each do |record|
...
write_records(travels_records, '_data/travel.json')
...
write_records(dictionary_records, '_data/dictionary.json')

## Media

media_records = collect_records('Media')
media_records.each do |record|
  next unless record['content']
...
# write_records(collect_records('Media'), '_data/media.json')
